...
write.csv(as.data.frame(permanova_mm_priinteract), "soil16s_perm.csv")



#simple
permanova_mm_priinteract_simple <- vegan::adonis2(as.dist(dist_pri)~meta_pri$weevil+
...
c_summary <- meta_pri %>% dplyr::group_by(site)%>%
  summarise_at("X21_mg.c.gsoil", funs(mean, std.error))


c <- ggplot(c_summary, aes(x= site, y = mean)) + 
  geom_point(size=4)  +
...
ph_summary <- meta_pri %>% dplyr::group_by(site)%>%
  summarise_at("pH2019", funs(mean, std.error))


ph <- ggplot(ph_summary, aes(x= site, y = mean)) + 
  geom_point(size=4)  +
  geom_errorbar(aes(ymin=mean- std.error,
                    ymax=mean+ std.error, 
                    width=0.4), position=dodge)+
...
dist <- psotu2veg(pri_ps) 


#nmds
#convert com to a matrix
m_com = as.matrix(dist)
...
#env dist
mp<- read.csv("metapri_env.csv")





en = envfit(nmds, mp, permutations = 999, na.rm = TRUE)
en
...
data.scores = as.data.frame(scores(nmds)$sites)
data.scores$Weevil = mp$Weevil.Presence
...
#plot of nmds no arrows
gg = ggplot(data = data.scores, aes(x = NMDS1, y = NMDS2)) + 
  geom_point(data = data.scores, aes(shape=Weevil, size = 3, alpha = 0.5)) + 
  
  theme(axis.title = element_text(size = 13, face = "bold", colour = "grey30"), 
...
#simple
permanova_mm_priinteract_simple <- vegan::adonis2(as.dist(dist_sec)~meta_sec$n.treat,
                                                  strata=meta_sec$block_)
...
data.scores = as.data.frame(scores(nmds)$sites)
data.scores$Nitrogen = mp$N_addition
...
#plot of nmds no arrows
gg = ggplot(data = data.scores, aes(x = NMDS1, y = NMDS2)) + 
  geom_point(data = data.scores, aes(shape=Nitrogen, size = 3, alpha = 0.5)) + 
  theme(axis.title = element_text(size = 13, face = "bold", colour = "grey30"), 
        panel.background = element_blank(), panel.border = element_rect(fill = NA, colour = "grey30"), 
        axis.ticks = element_blank(), axis.text = element_blank(), legend.key = element_blank(), 
        legend.title = element_text(size = 10, face = "bold", colour = "grey30"), 
        legend.text = element_text(size = 9, colour = "grey30")) +
  labs(colour = "Nitrogen")
